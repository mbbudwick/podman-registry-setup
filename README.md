...
Note: This was created with much starting, restarting, gratuitous cursing, googling, banging, pacing back and forth and several decent pours of bourbon on a Fedora 42 Server linux distribution.  This document started as a self instruction set for properly setting up a local container registry accessible by podman.  The registry will be secure and use trusted certificates.  I have since cleanedup the instructions ( ie: removed uneeded instructions that didnt work, put instructions in proper order, or explained what I was doing ). This document is meant as a map to get a local container registry that is somewhat secure. It wont delve into the details of ssl, podman, docker, certificates, etc.
...
Find the entry in the config as shown below.
...
# [[registry]]
```
Prepend your <server ip>:<server port> to the unqualified-search-registries field. Then below addd a  [[registry]] entry for that registry. There
should be none if this is your first time setting up, and there should be a commented out example in the config.  Your final conf file should have the same look as below except for your ip's and/or hostnames:

```bash
>unqualified-search-registries = [ "<your host name>:5000", "192.168.1.3:5000", "registry.fedoraproject.org", "registry.access.redhat.com", "docker.io"]
...
[[registry]]
location = "<your host name>:5000"
insecure = false
```

- What this does:
This sets up your registry to be searched in a default lookup.  This is evident when doing a podman/docker transaction and not specifying the registry or just using a short name.
...
Now we begin the process of creating the certificates. Self signed certificates already act as a self signed authority.  However what instruction set would be complete unless you follow through creating your very own self signed Certificate Authority (CA) for your local network.  Please make sure you put your own IP's in the file and not mine.
...
countryName = US
stateOrProvinceName = Someplace nice
localityName = RightWhereIam
...
```bash
>sudo openssl genrsa -out /opt/registry/certs/<hostname>.key 4096
```
...
```bash
>sudo openssl req -new -sha256 -key /opt/registry/certs/<hostname>.key -out /opt/registry/certs/<hostname>.csr --config /opt/registry/certs/san.cnf
```
...
```bash
>sudo openssl x509 -req -in /opt/registry/certs/<hostname>.csr -CA /opt/registry/certs/rootCA.crt -CAkey /opt/registry/certs/rootCA.key -CAcreateserial -out <hostname>.crt -days 5000 -sha256 -extfile san.cnf -extensions v3_req
```
...
>sudo cp /opt/registry/certs/<hostname>.crt /etc/pki/ca-trust/source/anchors/.
...
The entries here represent the server/location of your registry. If your registry and this machine are one and the same you don’t need to do anything further. You can skip the next sections of setup and move on to usage.  You will add references to your registry just as you did in the registries.conf on the server.  Below is how your .conf file should appear.

```bash
unqualified-search-registries = ["<hostname>:5000", "192.168.1.3:5000", "registry.fedoraproject.org", "registry.access.redhat.com", "docker.io"]
...
- What it does:
Again, this sets up podman to look for images in a default lookup when only a short name is specified for the image.
...
```bash
runroot = "/somplacenice/withlotsofmemory/containertemp"
```
...
```bash
graphroot = "/someplacenice/withlotsofmemory/containerstorage" 
```

- What this does:
Changes primary read/write location of container storage. Because this is a fedora distribution or because I make sure SELINUX is active, you must ensure the labeling matches the default locations labels with the following commands: 
...
```bash
>sudo scp -v user@<server ip>:/opt/registry/certs/<hostname>.crt /etc/containers/certs.d/<server ip>:port/ca.crt
...
>sudo cp /etc/containers/certs.d/192.168.1.3:5000/ca.crt /etc/pki/ca-trust/source/anchors/.
>sudo update-ca-trust
```
...
Contents of the file are listed just below
...
RUN groupadd qtdev 
RUN adduser --uid $UID --gid $GID -G qtdev <your user id> 
...
RUN chown -R <your user id>:qtdev /home/datadisk1/ContainerSources/Qt 

RUN mkdir -p /home/<your user id>/.cache/qt-unified-linux-online 
...
RUN echo '<your user id> ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers 
USER mbudwick 
...
```bash
>sudo podman run -it --rm --network=host --privileged -v /tmp/.X11-unix:/tmp/.X11-unix -v $XAUTHORITY:$XAUTHORITY -v ~/Development:/home/<your user id>/<your development dir> -e DISPLAY=$DISPLAY --security-opt label=type:container_runtime_t 192.168.1.3:5000/registry/myqtdevtainer:latest
```

- What this does:
Runs your image. --network says to use the hosts network. -v ~Development:/home/<your user id>/<your development dir> specifies the development directory volume that is mapped in the container to a directory name under the authorized user.  This can be local files or even files managed by source control.
